...
[7 Неструктурированные данные](#7)

[8 Подписанные данные](#8)

[8.1 Общее описание](#8.1)

[8.2 Тип `SignedData`](#8.2)

[8.3 Тип `EncapsulatedContentInfo`](#8.3)

[8.4 Тип `SignerInfo`](#8.4)

[8.5 Хэширование](#8.5)

[8.6 Выработка ЭЦП](#8.6)

[8.7 Проверка ЭЦП](#8.7)

[9 Конвертованные данные](#9)

[9.1 Общее описание](#9.1)

[9.2 Тип `EnvelopedData`](#9.2)

[9.3 Тип `RecipientInfo`](#9.3)

[9.4 Тип `KeyTransRecipientInfo`](#9.4)

[9.5 Процесс шифрования данных](#9.5)

[9.6 Процесс шифрования ключей](#9.6)

[10 Хэшированные данные](#10)

[10.1 Общее описание](#10.1)

[10.2 Тип `DigestedData`](#10.2)

[11 Шифрованные данные](#11)

[11.1 Общее описание](#11.1)

[11.2 Тип `EncryptedData`](#11.2)

[12 Аутентифицируемые данные](#12)

[12.1 Общее описание](#12.1)

[12.2 Тип `AuthenticatedData`](#12.2)

[12.3 Вычисление имитовставки](#12.3)

[12.4 Проверка имитовставки](#12.4)

[13 Аутентифицируемые конвертованные данные](#13)

[13.1 Общее описание](#13.1)

[13.2 Тип `AuthEnvelopedData`](#13.2)

[13.3 Шифрование и имитозащита](#13.3)

[13.4 Шифрование ключей](#13.4)

[14 Вспомогательные типы](#14)

[14.1 Тип `AlgorithmIdentifier` и связанные типы](#14.1)

[14.2 Тип `RevocationInfoChoices`](#14.2)

[14.3 Тип `CertificateChoices`](#14.3)

[14.4 Тип `CertificateSet`](#14.4)

[14.5 Тип `IssuerAndSerialNumber`](#14.5)

[14.6 Тип `CMSVersion`](#14.6)

[14.7 Тип `UserKeyingMaterial`](#14.7)

[14.8 Тип `OtherKeyAttribute`](#14.8)

[15 Атрибуты](#15)

[15.1 Общее описание](#15.1)

[15.2 Тип содержимого](#15.2)

[15.3 Хэш-значение](#15.3)

[15.4 Время подписания](#15.4)

[15.5 Контрподпись](#15.5)

[Приложение А (обязательное) Модули АСН.1](#91)

[Приложение Б (рекомендуемое) Использование криптографических алгоритмов в CMS](92)
...
[[1]](#99.1) (версия 1) и [[2]](#99.2) (версия 2).
...
шифрования к произвольным данным. Стандарт основан на [[2]](#99.2) и
его расширениях [[3]](#99.3), [[4]](#99.4).
...
# 7 <a name="7"></a>Неструктурированные данные
...
# 8 <a name="8"></a>Подписанные данные

## 8.1 <a name="8.1"></a>Общее описание
...
(см. [8.5](#8.5)). Результат хэширования объявляется окончательным
...
(см. [8.4](#8.4)). На этом же шаге собираются сертификаты и списки
...
подписывающих сторон (см. [8.2](#8.2)).
...
подписанных данных. В [8.5](#8.5), [8.6](#8.6), [8.7](#8.7)
описываются процессы хэширования, выработки и проверки ЭЦП.

## 8.2 <a name="8.2"></a>Тип SignedData
...
Процесс хэширования описан в [8.5](#8.5).
...
другие правила (см. [8.1](#8.1)).

Описание типа `SignerInfo` приведено в [8.4](#8.4).
...
## 8.3 <a name="8.3"></a>Тип EncapsulatedContentInfo
...
данные должны иметь тип `id-data` (см. раздел [7](#7)), а компонент
`eContent` ДОЛЖЕН быть опущен.

## 8.4 <a name="8.4"></a>Тип SignerInfo
...
Хэширование подробно описано в [8.5](#8.5). Используемый алгоритм
...
данных соответственно (см. [15.2](#15.2) и [15.3](#15.3)). Однако атрибут «тип
...
содержимого» и «хэш-значение» (см. [15.2](#15.2) и [15.3](#15.3)). Поэтому
...
## 8.6 <a name="8.6"></a>Выработка ЭЦП
...
хэшироваться по правилам, определенным в [8.5](#8.5). Для признания ЭЦП
...
# 9 <a name="9"></a>Конвертованные данные

## 9.1 <a name="9.1"></a>Общее описание
...
которые определяют формат конвертованных данных. В [9.4](#9.4),
[9.5](#9.5) описываются процессы шифрования данных и ключей.

## 9.2 <a name="9.2"></a>Тип EnvelopedData
...
| Включен компонент originatorInfo, который содержит атрибутный сертификат версии 2, или включен компонент RecipientInfo с выбором pwri или ori (см. [9.3](#9.3)) | 3 |
...
[9.4](#9.4). Одни и те же алгоритм и ключ шифрования данных 
...
## 9.3 <a name="9.3"></a>Тип RecipientInfo
...
## 9.4 <a name="9.4"></a>Тип KeyTransRecipientInfo
...
Процесс шифрования ключа шифрования данных описан в [9.5](#9.5).

- `encryptedKey` определяет зашифрованный ключ шифрования данных для
получателя.

### 9.4.1 <a name="9.4.1"></a>Тип KeyAgreeRecipientInfo
...
### 9.4.2 <a name="9.4.2"></a>Тип `KEKRecipientInfo`
...
### 9.4.3 <a name="9.4.3"></a>Тип `PasswordRecipientInfo`
...
### 9.4.4 <a name="9.4.4"></a>Тип `OtherRecipientInfo`
...
## 9.5 <a name="9.5"></a>Процесс шифрования данных
...
## 9.6 <a name="9.6"></a>Процесс шифрования ключей
...
# 10 <a name="10"></a>Хэшированные данные

## 10.1 <a name="10.1"></a>Общее описание
...
## 10.2 <a name="10.2"></a>Тип DigestedData
...
Тип данного компонента описан в [8.3](#8.3).

- `digest` содержит хэш-значение данных.

Хэширование производится в соответствии с [8.5](#8.5) для случая, когда
...
# 11 <a name="11"></a>Шифрованные данные

## 11.1 <a name="11.1"></a>Общее описание
...
## 11.2 <a name="11.2"></a>Тип EncryptedData
...
# 12 <a name="12"></a>Аутентифицируемые данные

## 12.1 <a name="12.1"></a>Общее описание
...
имитовставки данные (см. [9.3](#9.3)).
...
информации (см. [12.3](#12.3)), то вложенные данные хэшируются, полученное
...
## 12.2 <a name="12.2"></a>Тип `AuthenticatedData`
...
как описано в [9.2](#9.2).

- `recipientInfos` определяет набор связанной с получателем информации. Тип
данного компонента определен в [9.3](#9.3). Набор ДОЛЖЕН содержать по
...
хэш-значения производится в соответствии с [8.5](#8.5). Порядок следования
...
которых контролируется. Тип данного компонента определен в [8.3](#8.3).
...
атрибуты описаны в [15.2](#15.2) и [15.3](#15.3).
...
распространенные атрибуты описаны в разделе [15](#15).

## 12.3 <a name="12.3"></a>Вычисление имитовставки
...
содержимого» и «хэш-значение», определенные в [15.2](#15.2) и [15.3](#15.3).
...
## 12.4 <a name="12.4"></a>Проверка имитовставки
...
согласно [12.3](#12.3), и ключ имитозащиты, предаваемый в компоненте
...
определяются в [12.3](#12.3). Если отправитель включает аутентифицируемые
атрибуты, то имитовставка вычисляется от значения компонента `authAttrs`
согласно [12.3](#12.3). Кроме того, при наличии компонента `authAttrs`
...
# 13 <a name="13"></a>Аутентифицируемые конвертованные данные

## 13.1 <a name="13.1"></a>Общее описание
...
выработкой имитовставки данные (см. [9.3](#9.3)).
...
## 13.2 <a name="13.2"></a>Тип AuthEnvelopedData
...
компонента определен в [9.2](#9.2).
...
[12.2](#12.2). Данный компонент НЕ ДОЛЖЕН содержать атрибут «хэш-значение».
В разделе [15](#15) описаны некоторые распространенные атрибуты.
...
данных не используется. Тип данного компонента определен в [12.2](#12.2).

- `unauthAttrs` является необязательным компонентом, который определяет
неаутентифицируемые атрибуты. Тип данного компонента определен в [12.2](#12.2).
В разделе [15](#15) описаны некоторые распространенные атрибуты.

## 13.3 <a name="13.3"></a>Шифрование и имитозащита
...
дополняются до требуемой длины по правилам, определенным в [9.5](#9.5).
...
## 13.4 <a name="13.4"></a>Шифрование ключей

При шифровании ключей на вход алгоритма шифрования подается непосредственно ключ
шифрования данных, без атрибутов и информации о получателе. Любой из описанных в
[9.3](#9.3) методов управления ключами может быть применен для любого
получателя одних и тех же данных.

# 14 <a name="14"></a>Вспомогательные типы

## 14.1 <a name="14.1"></a>Тип `AlgorithmIdentifier` и связанные типы
...
(см. раздел [13](#13)), то он определяет алгоритмы одновременного
...
## 14.2 <a name="14.2"></a>Тип RevocationInfoChoices
...
## 14.3 <a name="14.3"></a>Тип CertificateChoices
...
## 14.4 <a name="14.4"></a>Тип `CertificateSet`
...
## 14.5 <a name="14.5"></a>Тип `IssuerAndSerialNumber`
...
## 14.6 <a name="14.6"></a>Тип `CMSVersion`
    
Тип `CMSVersion` определяет номер версии применяемого синтаксиса.
    
    CMSVersion ::= INTEGER {v0(0), v1(1), v2(2), v3(3), v4(4), v5(5)}

## 14.7 <a name="14.7"></a>Тип `UserKeyingMaterial`
...
ключей (см. [9.4.2](#9.4.2)).

    UserKeyingMaterial ::= OCTET STRING

## 14.8 <a name="14.8"></a>Тип `OtherKeyAttribute`
...
отправителем (см. [9.4.2](#9.4.2), [9.4.3](#9.4.3)). Следует
...
# 15 <a name="15"></a>Атрибуты

## 15.1 <a name="15.1"></a>Общее описание
...
Атрибуты могут быть подписываемыми (см. `signedAttrs` в [8.4](#8.4)),
неподписываемыми (см. `unsignedAttrs` в [8.4](#8.4)), аутентифицируемыми
(см. `authAttrs` в [12.2](#12.2) и [13.2](#13.2)),
неаутентифицируемыми (см. `unauthAttrs` в [12.2](#12.2) и
[13.2](#13.2)) и незашифрованными (см. `unprotectedAttrs` в
[9.2](#9.2) и [11.2](#11.2)).

Атрибуты описываются с помощью типа `Attribute` АСН.1 (см. [8.4](#8.4)).
...
## 15.2 <a name="15.2"></a>Тип содержимого
...
## 15.3 <a name="15.3"></a>Хэш-значение
...
(см. [8.5](#8.5)) или для которого вычисляется имитовставка
(см. [12.3](#12.3)). Для подписываемых данных алгоритм хэширования выбирается
...
## 15.4 <a name="15.4"></a>Время подписания
...
## 15.5 <a name="15.5"></a>Контрподпись
...
# Приложение А  <a name="91"></a>(обязательное)
# Модули АСН.1
...
# Приложение Б <a name="92"></a>(рекомендуемое)
...
компонента `signature` типа `SignerInfo` (см. [8.4](#8.4)) данное число
...
распределенные секретные ключи или пароли (см. [9.1](#9.1)).
...
правилами CMS (см. [8.5](#8.5)) на вход алгоритмов подаются
...
(см. [8.4](#8.4)) данная строка естественным образом представляется строкой
...
границу блока ДОЛЖЕН применяться метод, описанный в [9.5](#9.5).
...
[[5]](#99.5), описаны в СТБ П 34.101.50 (приложение В).

Определенный в [[5]](#99.5) протокол одностороннего формирования общего ключа
...
`KeyTransRecipientInfo` (см. [9.4.1](#9.4.1)).
