...
### [4 Общие положения](#4)
### [5 Синтаксис запроса на получение сертификата](#5)
### [Приложение А (обязательное) Модуль АСН.1](#91)
### [Приложение Б (обязательное) Отличительные правила кодирования](#92)
### [Приложение В (рекомендуемое) Запрос на получение сертификата открытого ключа электронной цифровой подписи согласно СТБ 1176.2](#93)
### [Библиография](#99)
...
выработанной на личном ключе пользователя (см. [5.2](#5.2)). 
...
## 5.1 <a name="5.1"></a> Тип `CertificationRequestInfo`
...
## <a name="5.2"></a>5.2 Тип `CertificationRequest`
...
отличительных правил кодирования (см. приложение [Б](#92)). 
...
# <a name="93"></a>Приложение В (рекомендуемое)
...
формирования общего ключа (ПФОК), определенных в [[2]](#99.2). 
...
ключом, соответствующим открытому ключу из запроса (в [[2]](#99.2) личный ключ
...
аутентификации сторон [[2]](#99.2) (пункт 4.1.4). При выработке общего ключа 
...
определенных в [В.2](#93.2). При создании доказательства пользователь 
должен применять алгоритм согласно [В.2.2](#93.2.2) и использовать личный 
ключ ПФОК, соответствующий открытому ключу. Для проверки владения личным 
ключом УЦ должен применять алгоритм согласно [В.2.3](#93.2.3) и 
использовать открытый ключ ПФОК пользователя из запроса. 

Компоненты типа `ElGamalSignature` имеют следующее значение:
- `w` – число _w_, вычисленное по алгоритму согласно [В.2.2](#93.2.2);
- `s` – число _s_, вычисленное по алгоритму согласно [В.2.2](#93.2.2).

## <a name="93.2"></a>В.2 Алгоритмы доказательства владения личным ключом протоколов формирования общего ключа
...
При описании алгоритмов используются обозначения согласно [[2]](#99.2). 
Дополнительно вводится символ _R_ = 2<sup>_l_ + 2</sup>. 

### <a name="93.2.2"></a>В.2.2 Алгоритм создания доказательства владения 
...
### <a name="93.2.3"></a>В.2.3 Алгоритм проверки доказательства владения личным ключом протоколов формирования общего ключа
