...
### [4 Общие положения](#4Common)
### [5 Синтаксис запроса на получение сертификата](#5Syntax)
### [Приложение А (обязательное) Модуль АСН.1](#99.1)
### [Приложение Б (обязательное) Отличительные правила кодирования](#99.2)
### [Приложение В (рекомендуемое) Запрос на получение сертификата открытого ключа электронной цифровой подписи согласно СТБ 1176.2](#99.3)
### [Библиография](#100)
...
# 3 <a name="3"></a>Термины и определения, обозначения
...
выработанной на личном ключе пользователя (см. [5.2](#52)). 
...
# 5 <a name="5"></a>Синтаксис запроса на получение сертификата

## 5.1 <a name="51"></a> Тип `CertificationRequestInfo`
...
## <a name="52"></a>5.2 Тип `CertificationRequest`
...
отличительных правил кодирования (см. приложение [Б](#992)). 
...
# <a name="99.1"></a>Приложение А (обязательное)
## Модуль АСН.1
...
# <a name="99.2"></a>Приложение Б (обязательное)
...
# <a name="99.3"></a>Приложение В (рекомендуемое)
...
формирования общего ключа (ПФОК), определенных в [[2]](#100.2). 
...
ключом, соответствующим открытому ключу из запроса (в [[2]](#100.2) личный ключ
...
аутентификации сторон [[2]](#100.2) (пункт 4.1.4). При выработке общего ключа 
...
определенных в [В.2](#99.32). При создании доказательства пользователь 
должен применять алгоритм согласно [В.2.2](#99.322) и использовать личный 
ключ ПФОК, соответствующий открытому ключу. Для проверки владения личным 
ключом УЦ должен применять алгоритм согласно [В.2.3](#99.323) и 
использовать открытый ключ ПФОК пользователя из запроса. 

Компоненты типа `ElGamalSignature` имеют следующее значение:
- `w` – число _w_, вычисленное по алгоритму согласно [В.2.2](#99.322);
- `s` – число _s_, вычисленное по алгоритму согласно [В.2.2](#99.322).

## <a name="99.32"></a>В.2 Алгоритмы доказательства владения личным ключом протоколов формирования общего ключа
...
### <a name="99.322"></a>В.2.2 Алгоритм создания доказательства владения 
...
### <a name="99.323"></a>В.2.3 Алгоритм проверки доказательства владения личным ключом протоколов формирования общего ключа
